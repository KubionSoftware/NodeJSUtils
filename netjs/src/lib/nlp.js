...
class NLP {
    ...
    static getKeywords (text) {
		text = Stemmer.replaceAccentedCharacters(text.toLowerCase());
		const words = text.split(/[^a-z]/g);
	
		const output = [];
		const originals = [];
		for (const word of words) {
			if (word.length > 0 && !stopwordSet.has(word)) {
				originals.push(word);
				output.push(Stemmer.stem(word))
			};
		}
	...
	static async loadClassificationModel (filePath) {
		const model = JSON.parse(await IO.readText(filePath));
		for (const subject in model.scores) model.scores[subject] = new Map(Object.entries(model.scores[subject]));
		return model;
	}
	
	static classify (model, text) {
		const words = NLP.getKeywords(text).keywords;

		let bestScore = 0;
		let bestSubject = undefined;
	
		for (const subject in model.scores) {
			const scores = model.scores[subject];
			let score = Math.log(model.distribution[subject]);
	
			for (const word of words) {
				if (scores.has(word)) {
					score += Math.log(scores.get(word));
				}
			}
