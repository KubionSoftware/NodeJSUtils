...
class NLP {
	...
	static classify (model, text) {
		const words = NLP.getKeywords(text.toLowerCase()).keywords;
		...
		for (let subject in model.scores) {
			let score = Math.log(model.distribution[subject]);
	
			for (let word of words) {
				if (word in model.scores[subject]) {
					score += Math.log(model.scores[subject][word]) * wordCount[word];
				}
			}
		...
		return scores.sort((a, b) => b.score - a.score)[0].subject;
	}
}
